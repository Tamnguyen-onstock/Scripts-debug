...
from prompts.intent_analysis import intent_prompt_template, INTENT_ANALYSIS
from prompts.information_extraction import info_prompt_template, INFORMATION_EXTRACTION
from prompts.question_type import question_type_prompt_template, QUESTION_TYPE
...
def perform_question_type_definition(query: str) -> Dict[str, Any]:
    """Define question type."""
    tool_name = "define_question_type"
    
    # Check cache first
    cached_result = query_cache.get(tool_name, query)
    if cached_result is not None:
        logger.info(f"Using cached result for question type definition: {query}")
        return cached_result
    
    logger.info(f"Cache miss, performing question type definition for: {query}")
    definition_start_time = time.time()
    
    try:
        system_prompt = QUESTION_TYPE
        user_prompt = question_type_prompt_template.format(query=query)
        ...
        try:
            ...
            logger.warning("Failed to parse JSON from question type definition response, retrying")
        ...
        definition_time = time.time() - definition_start_time
        logger.info(f"Question type definition completed in {definition_time:.2f} seconds")
        
        return result
    except Exception as e:
        logger.error(f"Error in question type definition: {str(e)}")
        definition_time = time.time() - definition_start_time
        logger.error(f"Definition failed after {definition_time:.2f} seconds")
